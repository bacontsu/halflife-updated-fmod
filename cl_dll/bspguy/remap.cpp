...
void STRUCTUSAGE::compute_sum()
{
	...
	for (int i = 0; i < count.planes; i++)
		sum.planes += planes[i];
	for (int i = 0; i < count.texInfos; i++)
		sum.texInfos += texInfo[i];
	for (int i = 0; i < count.leaves; i++)
		sum.leaves += leaves[i];
	for (int i = 0; i < count.nodes; i++)
		sum.nodes += nodes[i];
	for (int i = 0; i < count.clipnodes; i++)
		sum.clipnodes += clipnodes[i];
	for (int i = 0; i < count.verts; i++)
		sum.verts += verts[i];
	for (int i = 0; i < count.faces; i++)
		sum.faces += faces[i];
	for (int i = 0; i < count.textures; i++)
		sum.textures += textures[i];
	for (int i = 0; i < count.markSurfs; i++)
		sum.markSurfs += markSurfs[i];
	for (int i = 0; i < count.surfEdges; i++)
		sum.surfEdges += surfEdges[i];
	for (int i = 0; i < count.edges; i++)
		sum.edges += edges[i];
}
