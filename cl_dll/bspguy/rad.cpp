...
#pragma warning(disable : 4244) // conversion from 'double' to 'vec_t', possible loss of data
#pragma warning(disable : 4305) // truncation from 'int' to 'float'
...
static bool TestSampleFrag(Bsp* bsp, int facenum, vec_t s, vec_t t, const vec_t square[2][2], int maxsize)
{
	...
	VectorScale(v_s, 1, (vec_t*)&head.rect.planes[0].vNormal);
	head.rect.planes[0].fDist = square[0][0]; // smin
	VectorScale(v_s, -1, (vec_t*)&head.rect.planes[1].vNormal);
	head.rect.planes[1].fDist = -square[1][0]; // smax
	VectorScale(v_t, 1, (vec_t*)&head.rect.planes[2].vNormal);
	head.rect.planes[2].fDist = square[0][1]; // tmin
	VectorScale(v_t, -1, (vec_t*)&head.rect.planes[3].vNormal);
	head.rect.planes[3].fDist = -square[1][1]; // tmax
